...
#elif defined(__FASTBOOT__) || defined(__CRSLIBS__)
#include <libc.h>
#elif defined(__M3BOOT__)
#include <lib.h>
#elif defined(__RAMINIT__) || defined(__XLOADER__) || defined(__CORTEX_M3)
#include <string.h>
...
#define SECUREC_STRNCPY_SM(dest, destMax, src, count)                                                                                                                    \
    (((void *)(dest) != NULL && (void *)(src) != NULL && (size_t)(destMax) > 0 &&                                                                                        \
      ...
      (SECUREC_TWO_MIN((size_t)(count), strlen(src)) + 1) <= (size_t)(destMax))                                                                                          \
         ? (((size_t)(count) < strlen(src)) ? (memcpy((dest), (src), (count)), *((char *)(dest) + (count)) = '\0', EOK) : (memcpy((dest), (src), strlen(src) + 1), EOK)) \
