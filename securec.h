...
#elif defined __M3BOOT__
#elif defined __RAMINIT__
#elif defined __XLOADER__
#elif defined __UCE__
#elif defined __CRSLIBS__
#else
...
#define memcpy_sp(dest, destMax, src, count) (__builtin_constant_p((count)) ? (SECUREC_MEMCPY_SM((dest), (destMax), (src), (count))) : (__builtin_constant_p((destMax)) ? (((size_t)(destMax) > 0 && (((unsigned long long)(destMax) & (unsigned long long)(-2)) < SECUREC_MEM_MAX_LEN)) ? memcpy_sOptTc((dest), (destMax), (src), (count)) : ERANGE) : memcpy_sOptAsm((dest), (destMax), (src), (count))))

/* The memset_sp is a macro, not a function in performance optimization mode. */
#define memset_sp(dest, destMax, c, count) (__builtin_constant_p((count)) ? (SECUREC_MEMSET_SM((dest), (destMax), (c), (count))) : (__builtin_constant_p((destMax)) ? (((((unsigned long long)(destMax) & (unsigned long long)(-2)) < SECUREC_MEM_MAX_LEN)) ? memset_sOptTc((dest), (destMax), (c), (count)) : ERANGE) : memset_sOptAsm((dest), (destMax), (c), (count))))
#else
